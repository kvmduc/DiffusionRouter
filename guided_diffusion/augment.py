...
class AugmentPipe:
    ...
    def apply(self, images, arguments=None):
        """
        ...
        if self.xflip > 0:
            xflip_label = arguments[:, 0]
            images = torch.where(xflip_label.view(-1, 1, 1, 1) == 1, images.flip(3), images)
            labels += [xflip_label]
            
        if self.yflip > 0:
            yflip_label = arguments[:, 1]
            images = torch.where(yflip_label.view(-1, 1, 1, 1) == 1, images.flip(2), images)
            labels += [yflip_label]
        ...
        if self.translate_frac > 0:
            w_0 = arguments[:,6]
            w_1 = arguments[:,7]
            G_inv = G_inv @ translate2d_inv(w_0.mul(W * self.translate_frac_std), w_1.mul(H * self.translate_frac_std))
            labels += [w_0, w_1]
        ...
        return images, labels, arguments
    ...
    def __call__(self, images):
        ...
        device = images.device
        labels = [torch.zeros([images.shape[0], 0], device=device)]
        arguments = [torch.zeros([images.shape[0], 0], device=device)]
        ...
        labels = torch.cat([x.to(torch.float32).reshape(N, -1) for x in labels], dim=1)
        arguments = torch.cat([x.to(torch.float32).reshape(N, -1) for x in arguments], dim=1)
